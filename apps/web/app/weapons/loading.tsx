export default function WeaponsLoading() {
    return (
        <main className="bg-secondary">
            {/* Hero section skeleton — matches min-h-screen hero with text block */}
            <section className="flex min-h-screen w-full items-center bg-secondary px-8 md:px-16 lg:px-24">
                ...
                <div className="mx-auto w-full max-w-7xl animate-pulse">
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                        {Array.from({ length: 8 }).map((_, index) => (
                            <div key={index} className="h-80 w-full rounded-sm bg-white/10" />
                        ))}
                    </div>
                </div>
            </section>
